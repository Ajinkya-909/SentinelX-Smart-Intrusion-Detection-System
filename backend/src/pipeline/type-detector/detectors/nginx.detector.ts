...
export class NginxAccessDetector extends BaseDetector {
...
export class NginxErrorDetector extends BaseDetector {
  protected readonly logType = "NGINX_ERROR";
  protected readonly parserName = "nginxParserV1";
  ...
  protected readonly patterns: MicroPattern[] = [
    {
      ...
      regex: /^\d{4}\/\d{2}\/\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[(?:emerg|alert|crit|error|warn|notice|info|debug)\]\s+\d+#\d+:/,
      weight: 5,
      isCritical: true
    }
  ];
...
export const nginxAccessDetector = new NginxAccessDetector();
export const nginxErrorDetector = new NginxErrorDetector();